...
if sampleName.startswith("$") & runName.startswith("$"):
    connection = cx_Oracle.connect("msdba/msdba@node-win0:1521/biodb")
...
#
connection = cx_Oracle.connect("solid/solid@node-win0:1521/biodb")
cursor = connection.cursor()
...
# Load files to db.
#
command = "${sqlldr.path} solid/solid@node-win0:1521/biodb control=" + bpFilename + " log=" + bpSqlldrLogFilename
os.system(command)
command = "${sqlldr.path} solid/solid@node-win0:1521/biodb control=" + posFilename + " log=" + posSqlldrLogFilename
os.system(command)
