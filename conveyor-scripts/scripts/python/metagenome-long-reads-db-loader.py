...
inputFastqFile        = "${input.fastq.origin}"
...
try:
    fastqFileDecriptor = open(inputFastqFile, "rb")
except IOError:
    print "ERROR: Cannot open read set file"
    sys.exit()
...
if sampleName.startswith("$") & runName.startswith("$"):
    connection = cx_Oracle.connect("msdba/msdba@oracleHostAddress:1521/biodb")
...
#
# Chek for sampleName and runName properly initialization
#
if sampleName.startswith("$") & runName.startswith("$"):
...
#
connection = cx_Oracle.connect("solid/solid@oracleHostAddress:1521/biodb")
cursor = connection.cursor()
...
cursor.execute(
    "INSERT INTO mapping_runs (mapping_program, version, paramstring, ref_group_id) \
     VALUES('bowtie', '2.0.0-beta7', 'jsub-bowtie-module', :ref_group_id) \
...
#
# Count reads amount from fastq file.
#
nreads  = 0
pattern = re.compile("^[A-Z]+$")
for line in iter(fastqFileDecriptor):
    if pattern.match(line) is not None:
        nreads += 1
fastqFileDecriptor.close()
...
cursor.execute(
    ...
    {
        ":location" : inputFastqFile,
        ":nreads" : nreads,
        ":set_size" : os.path.getsize(fastqFileDecriptor.name),
...
# Load files to db.
#
command = "${sqlldr.path} solid/solid@oracleHostAddress:1521/biodb control=" + bpFilename + " log=" + bpSqlldrLogFilename
os.system(command)
command = "${sqlldr.path} solid/solid@oracleHostAddress:1521/biodb control=" + posFilename + " log=" + posSqlldrLogFilename
os.system(command)
...
#
# Call finalize oracle procedures for  for HMP for taxonomic (HMP).
#
if refGroupId in hmpReferences:
    ...
    cursor.callproc("insert_mapping_read_sets", [ mappingRunId, 182 ])
